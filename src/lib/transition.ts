...
/**
 * The `enterExit` function is a TypeScript function that creates a transition configuration for
 * animating the entrance and exit of an element based on specified options.
 * @param {Element} node - The `node` parameter is the element that you want to apply the enter/exit
 * transition to.
 * @param options - - `start`: The starting position of the transition. It can be one of the following
 * values: 'top', 'bottom', 'left', 'right'.
 * @returns The function `enterExit` returns a `TransitionConfig` object.
 */
export const enterExit = (
	node: Element,
	options: transitionOptions & inOutOptions = { start: 'top', moveY: true }
): TransitionConfig => {
	const scaleDir = scaleDirMap[options.start];
	const transformOrigin = transformOriginMap[options.start];

	const { borderRadius, boxShadow } = getComputedStyle(node);
	const radius = parseSize(borderRadius);
	function getClipPath(n: string) {
		const out = boxShadow != 'none' ? '-100%' : '0';
		switch (options.start) {
			case 'bottom':
				return `${n} ${out} -100% ${out}`;
			case 'left':
				return `${out} ${n} ${out} -100%`;
			case 'right':
				return `${out} -100% ${out} ${n}`;
			default:
				return `-100% ${out} ${n} ${out}`;
		}
	}
	function getTransform(u = 1) {
		if (!options.moveY) return '';
		switch (options.start) {
			case 'bottom':
				return `translateY(${u * 10}%)`;
			case 'left':
				return `translateX(${u * -10}%)`;
			case 'right':
				return `translateX(${u * 10}%)`;
			default:
				return `translateY(${u * -10}%)`;
	...
	return {
		...
		css: (t, u) => {
			const scale = (t * 0.3 + 0.7) * 100;
			const opacity = Math.min(t * 3, 1);
			const transform = getTransform(u);
			const clipPath = getClipPath((boxShadow !== 'none' ? u * 110 - 10 : u * 100) + '%');

			return `clip-path: inset(${clipPath} round ${radius}px);
                transform-origin: ${transformOrigin};
                transform: scale${scaleDir}(${scale}%) ${transform};
                opacity: ${opacity};`;
		}
	};
};

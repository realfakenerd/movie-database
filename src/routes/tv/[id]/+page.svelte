<script lang="ts">
	...
	const { tv, config } = data;

	const trailer = tv.videos.results.find((m) => m.type === 'Trailer');
	...
	const profile_path_srcset = (str) => `
		...
		`;
	const logo_path_srcset = (str) => `
		${getImagePath('logo', 0, str, config)} 45w,
		${getImagePath('logo', 1, str, config)} 92w,
		${getImagePath('logo', 2, str, config)} 154w,
		${getImagePath('logo', 3, str, config)} 185w,
		${getImagePath('logo', 4, str, config)} 300w,
		${getImagePath('logo', 5, str, config)} 500w,
		`;
</script>
...
<section
	...
	style="background-image: url({tv.backdrop_path
		? getImagePath('backdrop', 3, tv.backdrop_path, config)
			...
			<div class="col-span-3 grid grid-cols-3 gap-10">
				...
				>
					<div class="stat place-items-center">
						<div class="stat-title">tv release date</div>
						<div class="stat-value">{formatDate(tv.first_air_date)}</div>
						<div class="stat-desc">tv status: <span class="badge">{tv.status}</span></div>
					</div>
			...
			<div class="flex flex-col justify-around items-center w-full">
				<section class="w-full">
					...
					<div class="w-full inline-flex gap-5">
						...
						<p>base on {tv.vote_count} votes</p>
					</div>
				</section>
				<section>
					...
					<label for="my-modal-4" class="modal modal-bottom cursor-pointer sm:modal-middle">
						<label class="modal-box bg-base-100 relative min-w-fit" for="">
							{#if trailer}
								<iframe
									width="640"
									height="480"
...
<section id="mainHeroContainer" class="flex flex-col-reverse gap-5 md:flex-row p-10 ">
	<div class="flex flex-col gap-4 md:w-1/2">
		<section class="w-full">
			<h2 class="text-2xl font-semibold">Created by</h2>
			<ul class="flex flex-wrap">
				{#each tv.created_by as creator (creator.id)}
					<li
						class="group rounded-box flex w-full sm:w-1/2 items-center gap-3 p-2 transition-colors duration-300 hover:bg-base-200"
					>
						<figure class="avatar">
		...
		<section class="w-full">
			...
			<ul class="carousel carousel-start bg-gradient-to-r from-base-200 p-4 space-x-4 rounded-box">
				{#each tv.credits.cast.slice(0, 14) as cast (cast.id)}
					<li
	...
	<div class="space-y-5 md:w-1/2">
		...
		<div class="carousel carousel-center rounded-box space-x-4 bg-gradient-to-r from-base-200 p-4">
			<div class="carousel-item">
				{#each tv.videos.results.slice(0, 3) as video}
					<div class="p-2">
						<h3>{video.name}</h3>
						<iframe
							class="rounded-box"
